...
#include "video/driver/EVideoDriverFeatures.h"
...
namespace irrgame
{
	namespace video
	{
		class SharedOpenGLExtensionHandler
		{
			public:
				//! Singleton realization
				static SharedOpenGLExtensionHandler& getInstance();

			private:
				//! Default constructor. Should use only one time.
				SharedOpenGLExtensionHandler();

				//! Destructor. Should use only one time.
				virtual ~SharedOpenGLExtensionHandler();

				//! Copy constructor. Do not implement.
				SharedOpenGLExtensionHandler(
						const SharedOpenGLExtensionHandler& root);

				//! Override equal operator. Do not implement.
				const SharedOpenGLExtensionHandler& operator=(
						SharedOpenGLExtensionHandler&);

			public:

				// deferred initialization
				virtual void initExtensions(bool stencilBuffer);

				//! queries the features of the driver, returns true if feature is available
				virtual bool queryFeature(EVideoDriverFeatures feature) const;
			...
			public:
				...
				// general functions
				virtual void extGlActiveTexture(GLenum texture);

			public:
				//TODO: move to struct or create private
