...
//				{
//					CFRange range =
//					{	0, CFArrayGetCount ((CFArrayRef)refElementTop)};
...
//					CFRange range =
//					{	0, CFArrayGetCount ((CFArrayRef)refCFTopElement)};
...
//! constructor
	...
	: CIrrDeviceStub(param), Window(NULL), IsActive(true), OGLContext(NULL),
	...
	IsShiftDown(false), IsControlDown(false), MouseButtonStates(0)
	{
		...
		if (firstLaunch)
		{
			...
			if(!CreationParams.WindowId) //load menus if standalone application
			{
				...
				SEND_OBJC_MESSAGE(SEND_OBJC_MESSAGE(CLASS("NSAutoreleasePool"), "alloc"), "init");
				SEND_OBJC_MESSAGE(CLASS("NSApplication"), "sharedApplication")
				id delegate = SEND_OBJC_MESSAGE_WITH_OBJECT(SEND_OBJC_MESSAGE(CLASS("AppDelegate"), "alloc"), "initWithDevice:", this);
				SEND_OBJC_MESSAGE_WITH_OBJECT(CLASS("NSApp"), "setDelegate:", delegate);
				SEND_OBJC_MESSAGE_WITH_OBJECT(CLASS("NSBundle"), "loadNibNamed:owner:", CFSTR("MainMenu"), SEND_OBJC_MESSAGE(CLASS("NSApp"), "delegate"));
...
//				[NSApplication sharedApplication];
			...
			path = SEND_OBJC_MESSAGE(SEND_OBJC_MESSAGE(SEND_OBJC_MESSAGE(CLASS("NSBundle"), "mainBundle"), "bundlePath"), "stringByDeletingLastPathComponent");
			chdir(SEND_OBJC_MESSAGE(path, "fileSystemRepresentation"));
	...
	bool CIrrDeviceMacOSX::createWindow()
	{
		...
		if (!CreationParams.Fullscreen)
		{
			...
			{
				Window = SEND_OBJC_MESSAGE(CLASS("NSWindow"), "alloc");
				NSRect rect = NSMakeRect(0,0,CreationParams.WindowSize.Width,CreationParams.WindowSize.Height);
				NSInteger styleMask = NSTitledWindowMask + NSClosableWindowMask+NSResizableWindowMask;
				Window = SEND_OBJC_MESSAGE_WITH_OBJECT(Window, "initWithContentRect:styleMask:backing:defer:", rect, styleMask, NSBackingStoreBuffered, FALSE);
			...
			if (Window != NULL || CreationParams.WindowId)
			{
				...
				for (int i=0; i<3; ++i)
				{
					...
					format = SEND_OBJC_MESSAGE_WITH_OBJECT(SEND_OBJC_MESSAGE(CLASS("NSOpenGLPixelFormat"), "alloc"), "initWithAttributes:", windowattribs);
				...
				if (format != NULL)
				{
					OGLContext = SEND_OBJC_MESSAGE_WITH_OBJECT(SEND_OBJC_MESSAGE(CLASS("NSOpenGLPixelFormat"), "alloc"), "initWithAttributes:shareContext:", format, NULL);
				...
				if (OGLContext != NULL)
				{
					if (!CreationParams.WindowId)
					{
						...
						//[Window center];
						SEND_OBJC_MESSAGE_WITH_OBJECT(Window, "setDelegate:", SEND_OBJC_MESSAGE(CLASS("NSApp"), "delegate"));
						//[Window setDelegate:[NSApp delegate]];
						SEND_OBJC_MESSAGE_WITH_OBJECT(OGLContext, "setView:", SEND_OBJC_MESSAGE(Window, "contentView"));
						//[OGLContext setView:[Window contentView]];
						SEND_OBJC_MESSAGE_WITH_OBJECT(Window, "setAcceptsMouseMovedEvents:", true);
					...
					{ //use another window for drawing

						SEND_OBJC_MESSAGE_WITH_OBJECT(OGLContext, "setView:", CreationParams.WindowId);
						//[OGLContext setView:(NSView*)CreationParams.WindowId];
					}

					CGLContext = SEND_OBJC_MESSAGE(OGLContext, "CGLContextObj");
					// (CGLContextObj) [OGLContext CGLContextObj];
