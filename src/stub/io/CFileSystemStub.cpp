...
namespace irrgame
{
	namespace io
	{
		...
		//! constructor
		CFileSystemStub::CFileSystemStub()
		{
#ifdef DEBUG
		...
		IReadFile* CFileSystemStub::createAndOpenFile(const io::path& filename)
		{
			IReadFile* file = 0;
			u32 i;
		...
		IReadFile* CFileSystemStub::createMemoryReadFile(void* memory, s32 len,
		...
		IReadFile* CFileSystemStub::createLimitReadFile(const io::path& fileName,
				IReadFile* alreadyOpenedFile, long pos, long areaSize)
		{
			if (!alreadyOpenedFile)
				return 0;
			else
				return irrgame::io::createLimitReadFile(fileName,
						alreadyOpenedFile, pos, areaSize);
		}

		//! Creates an IReadFile interface for treating memory like a file.
		IWriteFile* CFileSystemStub::createMemoryWriteFile(void* memory, s32 len,
				const io::path& fileName, bool deleteMemoryWhenDropped)
		{
			if (!memory)
				return 0;
		...
		IWriteFile* CFileSystemStub::createAndWriteFile(const io::path& filename,
				bool append)
		{
...
//						ELL_ERROR);
		...
		const io::path& CFileSystemStub::getWorkingDirectory()
		{
...
//			}
		...
		bool CFileSystemStub::changeWorkingDirectoryTo(const io::path& newDirectory)
		{
			bool success = false;
...
//			}
		...
		io::path CFileSystemStub::getAbsolutePath(const io::path& filename) const
		{
//#if defined(_IRR_WINDOWS_CE_PLATFORM_)
//			return filename;
//#elif defined(_IRR_WINDOWS_API_)
...
//#endif
		...
		//! is returned.
		io::path CFileSystemStub::getFileDir(const io::path& filename) const
		{
		...
		io::path& CFileSystemStub::flattenFilename(io::path& directory,
				const io::path& root) const
		{
			directory.replace('\\', '/');
		...
		EFileSystemType CFileSystemStub::setFileListSystem(EFileSystemType listType)
		{
		...
		//! Creates a list of files and directories in the current working directory
		IFileList* CFileSystemStub::createFileList()
		{
//			IFileList* r = 0;
...
//				r = irrgame::io::createFileList(Path, false, false);
////				r = new CFileList(Path, false, false);
//
...
//					{
//						if (core::isInSameDirectory(Path,
//								merge->getFullFileName(j)) == 0)
//						{
//							io::path fullPath = merge->getFullFileName(j);
//							r->addItem(fullPath, merge->getFileSize(j),
...
//			return r;
		...
		bool CFileSystemStub::existFile(const io::path& filename) const
		{
//			for (u32 i = 0; i < FileArchives.size(); ++i)
...
//#endif
		...
		IXMLReader* CFileSystemStub::createXMLReader(const io::path& filename)
		{
...
//		}
		...
		{
			IWriteFile* file = this->createAndWriteFile(filename);
